...
5. через 6 ч проверяет, не вышли ли новые прогоны погоды, и **пересчитывает прогноз** (версия 2).
...
|---|---|---|---|
| **WindAgent (наша модель)** | **0.169** | 0.229 | 0.159 |
...
Улучшение MAE на сутки вперёд: 48.0% vs климатологии, 54.1% vs персистентности, 10.8% vs кривой мощности NWP.
Интервал P10–P90: покрытие до калибровки 0.716; коэффициент расширения k = 1.06 (подобран на 2025-10, 2025-11, 2025-12); проверка на отложенном месяце 2026-01: 0.732 → **0.807** (цель 0.80).
Перенос на «новую» турбину: обучение только на t1 → MAE t2 = 0.180 (при обучении на обеих турбинах 0.177).
...
Пост-проверка выпуска 31.01 (внутрисуточная часть, факт есть): MAE 0.190 за 24 ч.
...
| **Внутрисуточная коррекция** по свежим данным SCADA (только если данные моложе 2 ч) | [`forecast.py`](src/windagent/forecast.py) | `run.json` → `nowcast` |
| **Пересчёт при обновлении данных.** Проверка новых прогонов через 6 ч, версия 2, оценка величины пересмотра | `check_for_updates` в [`tools.py`](src/windagent/agent/tools.py) | `run.json` → `versions`, `kpis.revision_mae_v2_vs_v1` |
...
-- часы симуляции +6 ч --
check_for_updates    → вышли ли новые прогоны?  да → fetch → validate → run → analyze → publish v2
```
...
- Задержки — это задержки публикации на Open-Meteo, измеренные по `meta.json`, с запасом (таблица в разделе 9).
...
- **Решает LLM:** порядок действий, исключение моделей по флагам проверки, повторный запуск, пересчёт при обновлениях, текст аналитики.
...
|---|---|---|
| Язык | Python 3.11–3.13 | весь проект |
...
2. **Один выпуск с полной трассировкой агента:**
   ...
   - через 6 ч: «4 model(s) published newer runs» → пересчёт → v2;
   - в конце аналитика и «Пост-проверка» по факту 31.01 (MAE около 0.19).
...
- **Часы данных — фиксированный UTC+6.** Регистратор не перешёл на UTC+5 при смене часового пояса 01.03.2024. Доказательства воспроизводит `detect-clock`:
  ...
  - корреляция с прогнозами погоды максимальна при сдвиге +6.
...
- Сдвиги `previous_day1…4`. Семантика проверена сверкой значений с отдельными прогонами: значение на час v — из прогона, начатого в `floor_6ч(v) − N суток`.
...
SDUTIA. Разработка велась в этом репозитории в рамках 5-часового ограничения хакатона; история коммитов отражает ход работы.
