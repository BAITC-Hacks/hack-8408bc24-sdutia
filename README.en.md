...
Agent logs: 834 LLM events; the argument validator rejected 4 malformed LLM tool call(s) (2026-02-08_0000, 2026-02-12_0000, 2026-02-18_0000), which the LLM then repeated correctly; automatic fallbacks to the rules policy: 1 (2026-02-08_0000).
...
| **Dashboard** (Streamlit), RU/EN: project explanation and key results; tabs "Forecast", "Agent" (run the agent with a log of steps and decisions), "Validation", "Test period", "How it works" (with a "Run it yourself" block) | [`app/`](app/) | [`tests/ui/`](tests/ui/), [`docs/figures/ui/`](docs/figures/ui/) |
...
- **A real LLM decision** (issue of 10.02, 06:00 update, text written by `gpt-5.4-mini`, file `outputs/shelek/runs/2026-02-10_0000/trace.jsonl`, in Russian): «На 2026-02-10 06:00 зафиксированы новые входы: для ecmwf_aifs025_single, ecmwf_ifs025 и gfs_seamless обновлено 28% оставшихся часов, для icon_seamless — 16%, поэтому прогноз пересчитан.» ("At 06:00 on 10 Feb, new inputs were recorded: 28% of the remaining hours were updated for ECMWF AIFS, ECMWF IFS and GFS, and 16% for ICON, so the forecast was recomputed.")
- **The guardrails fired in real runs:** malformed LLM tool calls were rejected by the argument validator and repeated by the model; on an API timeout the rules policy completed the work (counts are in the results block).
...
- It is the same image that `docker compose up --build` builds from the repository (section 7); the deployed version does not replace running it yourself.

| Forecast | Agent | Validation |
|---|---|---|
| ![Forecast](docs/figures/ui/forecast.png) | ![Agent](docs/figures/ui/agent.png) | ![Validation](docs/figures/ui/validation.png) |
